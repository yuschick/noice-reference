...
export type Dota2GSIEvent = {
  ...
  abilities?: {[key: string]: Dota2GSIEventAbility}
  items?: {[key: string]: Dota2GSIEventItem}
}
...
export class GSIService {
  static SendDota2Event(req: Dota2GSIEvent, initReq?: fm.InitReq): Promise<GoogleProtobufEmpty.Empty> {
    return fm.fetchReq<Dota2GSIEvent, GoogleProtobufEmpty.Empty>(`/v1/gsi/dota2/events`, {...initReq, method: "POST", body: JSON.stringify(req)})
  }
  static StreamDota2Events(req: StreamEventsRequest, entityNotifier?: fm.NotifyStreamEntityArrival<Dota2GSIEvent>, initReq?: fm.InitReq): Promise<void> {
    if (initReq?.forceWebSocket) {
      return fm.webSocketStreamingRequest<StreamEventsRequest, Dota2GSIEvent>(`/v1/gsi/dota2/events:stream?${fm.renderURLSearchParams(req, [])}`, entityNotifier, {...initReq, method: "GET"})
    }
    return fm.fetchStreamingRequest<StreamEventsRequest, Dota2GSIEvent>(`/v1/gsi/dota2/events:stream?${fm.renderURLSearchParams(req, [])}`, entityNotifier, {...initReq, method: "GET"})
  }
}