...
export enum ReportCaseResolution {
...
export enum ReportContextUserTarget {
  TARGET_UNSPECIFIED = "TARGET_UNSPECIFIED",
  TARGET_NAME = "TARGET_NAME",
}

export enum ReportContextChannelTarget {
  TARGET_UNSPECIFIED = "TARGET_UNSPECIFIED",
  TARGET_NAME = "TARGET_NAME",
...
export type ReportContext = BaseReportContext
  & OneOf<{ user: ReportContextUser; channel: ReportContextChannel; stream: ReportContextStream; chatMessage: ReportContextChatMessage }>
...
export function routeReportContextValueDelegate<C>(ctx: C, val: ReportContext, delegate: IReportContextValueDelegate<C>) {
  val?.user && delegate.onUser(ctx, val.user)
  val?.channel && delegate.onChannel(ctx, val.channel)
  val?.stream && delegate.onStream(ctx, val.stream)
...
export class SupportService {
  static CreateTicket(req: CreateTicketRequest, initReq?: fm.InitReq): Promise<GoogleProtobufEmpty.Empty> {
    return fm.fetchReq<CreateTicketRequest, GoogleProtobufEmpty.Empty>(`/v1/support/tickets`, {...initReq, method: "POST", body: JSON.stringify(req)})
  }
  static GetArticle(req: GetArticleRequest, initReq?: fm.InitReq): Promise<Article> {
    return fm.fetchReq<GetArticleRequest, Article>(`/v1/support/articles/${req["locale"]}/${req["id"]}?${fm.renderURLSearchParams(req, ["locale", "id"])}`, {...initReq, method: "GET"})
  }
  static GetAttachment(req: GetAttachmentRequest, entityNotifier?: fm.NotifyStreamEntityArrival<GoogleApiHttpbody.HttpBody>, initReq?: fm.InitReq): Promise<void> {
    if (initReq?.forceWebSocket) {
      return fm.webSocketStreamingRequest<GetAttachmentRequest, GoogleApiHttpbody.HttpBody>(`/v1/support/attachments/${req["locale"]}/${req["id"]}?${fm.renderURLSearchParams(req, ["locale", "id"])}`, entityNotifier, {...initReq, method: "GET"})
    }
    return fm.fetchStreamingRequest<GetAttachmentRequest, GoogleApiHttpbody.HttpBody>(`/v1/support/attachments/${req["locale"]}/${req["id"]}?${fm.renderURLSearchParams(req, ["locale", "id"])}`, entityNotifier, {...initReq, method: "GET"})
  }
  static CreateZendeskToken(req: GoogleProtobufEmpty.Empty, initReq?: fm.InitReq): Promise<CreateZendeskTokenResponse> {
    return fm.fetchReq<GoogleProtobufEmpty.Empty, CreateZendeskTokenResponse>(`/v1/support/zendesk:token`, {...initReq, method: "POST"})
  }
  static CreateReport(req: CreateReportRequest, initReq?: fm.InitReq): Promise<Report> {
    return fm.fetchReq<CreateReportRequest, Report>(`/v1/support/reports`, {...initReq, method: "POST", body: JSON.stringify(req)})
  }
  static ListReports(req: ListReportsRequest, initReq?: fm.InitReq): Promise<ListReportsResponse> {
    return fm.fetchReq<ListReportsRequest, ListReportsResponse>(`/v1/support/reports?${fm.renderURLSearchParams(req, [])}`, {...initReq, method: "GET"})
  }
  static GetReportCase(req: GetReportCaseRequest, initReq?: fm.InitReq): Promise<ReportCase> {
    return fm.fetchReq<GetReportCaseRequest, ReportCase>(`/v1/support/cases/${req["id"]}?${fm.renderURLSearchParams(req, ["id"])}`, {...initReq, method: "GET"})
  }
  static BatchGetReports(req: BatchGetReportsRequest, initReq?: fm.InitReq): Promise<BatchGetReportsResponse> {
    return fm.fetchReq<BatchGetReportsRequest, BatchGetReportsResponse>(`/v1/support/reports:batchGet?${fm.renderURLSearchParams(req, [])}`, {...initReq, method: "GET"})
  }
  static ResolveReportCase(req: ResolveReportCaseRequest, initReq?: fm.InitReq): Promise<GoogleProtobufEmpty.Empty> {
    return fm.fetchReq<ResolveReportCaseRequest, GoogleProtobufEmpty.Empty>(`/v1/support/cases/${req["id"]}:resolve`, {...initReq, method: "POST", body: JSON.stringify(req)})
  }
}