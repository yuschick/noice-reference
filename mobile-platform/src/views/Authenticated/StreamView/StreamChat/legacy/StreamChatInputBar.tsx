...
import { AutomodState } from '@hooks/chat/types/moderation';
...
interface Props {
  ...
  onDrawerLayoutChanged?: (height: number) => void;
...
export const StreamChatInputBar = forwardRef(
  ({ message, onMessageChange, onDrawerLayoutChanged, onSend }: Props, ref) => {
    ...
    useEffect(() => {
      ...
      if (isKeyboardVisible) {
        onDrawerLayoutChanged?.(0);
      }
    }, [isKeyboardVisible, onDrawerLayoutChanged]);
    ...
    const toggleEmojiDrawer = () => {
      setIsEmojiDrawerVisible((prev) => {
        const enabled = !prev;
        onDrawerLayoutChanged?.(enabled ? emojiDrawerHeight : 0);
